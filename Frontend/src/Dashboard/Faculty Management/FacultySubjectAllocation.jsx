...
export default function FacultySubjectAllocation() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = editingId
        ? await updateSubjectAssignment(editingId, form)
      ...
      if (editingId) {
        setAllocations((list) => list.map((item) => (item.id === editingId ? allocation : item)));
      } else {
        setAllocations((list) => [allocation, ...list]);
      }
    } catch (error) {
      console.error("Unable to save subject allocation:", error);
      alert(error.response?.data?.message || "Unable to save subject allocation");
      return;
    }
    resetForm();
  };
  ...
  const deleteRow = async (id) => {
    ...
    try {
      ...
      console.error("Unable to delete subject allocation:", error);
      alert(error.response?.data?.message || "Unable to delete subject allocation");
    }
  };
