...
import { CalendarDays, ClipboardClock, Pencil, PieChart, UserCheck, UserX, Users } from "lucide-react";
...
function StudentSummary({ rows }) { const s = rows.flatMap((r) => [studentSessionStatus(r, "morning"), studentSessionStatus(r, "afternoon")]).filter((x) => x !== "—"), p = s.filter((x) => x === "Present").length; return <Summary student data={[["Total Students", rows.length], ["Present", p], ["Absent", s.filter((x) => x === "Absent").length], ["Leave", s.filter((x) => x === "Leave").length], ["Attendance %", s.length ? `${Math.round(p * 100 / s.length)}%` : "0%"]]} />; }
