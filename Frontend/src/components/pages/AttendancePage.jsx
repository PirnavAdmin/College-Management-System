...
const STUDENT_IMPORT_ERRORS = [[12, "ADM-014", "Ravi", "01-09-2026", "Attendance already exists"], [24, "ADM-022", "Asha", "02-09-2026", "Invalid Section"], [39, "ADM-041", "Nikhitha", "03-09-2026", "Invalid Attendance Status"], [54, "ADM-055", "Arshad", "04-09-2026", "Duplicate row in Excel"], [76, "ADM-071", "Ramya", "05-09-2026", "Student not found"], [92, "ADM-089", "Vidya", "06-09-2026", "Invalid Period"]];
const STAFF_IMPORT_ERRORS = [[8, "FAC-101", "Ravi Kumar", "01-09-2026", "Attendance already exists"], [21, "FAC-114", "Priya", "02-09-2026", "Invalid Check-Out Time"], [34, "FAC-127", "Arjun", "03-09-2026", "Staff not found"], [49, "FAC-132", "Kavya", "04-09-2026", "Invalid Attendance Status"]];
...
function Screen({ staff = false, say }) {
 ...
 const visible = rows.filter((r) => {
   ...
   return [get(r, "studentName", "name"), get(r, "rollNo", "rollNumber"), get(r, "admissionNo", "admissionNumber")]
...
function DailyTable({ rows, staff, edit, view, emptyMessage }) {
  ...
  const admission = !staff && rows.some((row) => get(row, "admissionNo", "admissionNumber") != null);

  return <div className="att-scroll"><table className="cms-table att-table"><thead><tr>{staff ? <><th>Staff ID</th><th>Staff Name</th><th>Department</th><th>Designation</th><th>Status</th><th>In Time</th><th>Out Time</th></> : <><th>Student Name</th>{roll ? <th>Roll No</th> : null}{admission ? <th>Admission No</th> : null}<th>Group</th><th>Section</th><th>Morning</th><th>Afternoon</th></>}<th>Action</th></tr></thead><tbody>{rows.length ? rows.map((row, index) => {
    const studentName = get(row, "studentName", "name") || "—";
    const staffName = get(row, "staffName", "facultyName") || "—";
    return <tr key={get(row, staff ? "facultyId" : "studentId", "id") ?? index}>{staff ? <><td>{get(row, "facultyId", "staffId")}</td><td><button type="button" className="attendance-staff-link" onClick={() => view(row)}>{staffName}</button></td><td>{get(row, "departmentName", "department") || "—"}</td><td>{get(row, "designationName", "staffTypeName") || "—"}</td><td><Pill value={status(row.status)} /></td><td>{get(row, "inTime") || "—"}</td><td>{get(row, "outTime") || "—"}</td></> : <><td><button type="button" className="attendance-student-link" onClick={() => view(row)}>{studentName}</button></td>{roll ? <td>{get(row, "rollNo", "rollNumber") || "—"}</td> : null}{admission ? <td>{get(row, "admissionNo", "admissionNumber") || "—"}</td> : null}<td>{get(row, "groupName") || "—"}</td><td>{get(row, "sectionName") || "—"}</td><td><Pill value={studentSessionStatus(row, "morning")} /></td><td><Pill value={studentSessionStatus(row, "afternoon")} /></td></>}<td><div className="att-row-actions"><button className="cms-action-btn" title="Edit Attendance" aria-label="Edit Attendance" onClick={() => edit(row)}><Pencil size={16} /></button></div></td></tr>;
  }) : <tr><td colSpan="9"><div className="cms-empty">{emptyMessage || "No attendance records match the selected filters."}</div></td></tr>}</tbody></table></div>;
}
