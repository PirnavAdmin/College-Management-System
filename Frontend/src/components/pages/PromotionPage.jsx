...
export default function PromotionPage() {
  ...
  const loadMasters = useCallback(async () => {
    ...
    try {
      ...
      const levels = unique(levelItems.map((item) => typeof item === "string" ? item : read(item, "academicLevelName", "AcademicLevelName", "academicLevel", "AcademicLevel", "levelName", "LevelName", "name", "Name")));
      const groups = groupItems.map((item) => ({
        ...
        level: asString(read(item, "academicLevelName", "AcademicLevelName", "academicLevel", "AcademicLevel", "levelName", "LevelName", "level", "Level")),
      ...
      const levelName = (item) => read(item, "academicLevelName", "AcademicLevelName", "academicLevel", "AcademicLevel", "levelName", "LevelName", "level", "Level");
      const derivedLevels = unique([...levels, ...groupItems.map(levelName), ...sectionItems.map(levelName)]);
      setMasters({ years, boards, levels: derivedLevels.map((item) => option(item)), groups, sections });
  ...
  const eligibleParams = useCallback(() => compactParams({
    ...
    GroupId: numericId(setup.group), Section: setup.fromSection,
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const response = await apiClient.get(promotionApi.history, { params: compactParams({
        AcademicYearId: numericId(historyFilters.academicYearId),
    ...
    <DashboardLayout title="Promotion Management" subtitle="Review eligibility, promote students, and manage promotion history." breadcrumb={["Academics", "Promotion"]}>
      <div className="promotion-page">
        ...
        {activeTab === "promotion" ? <>
          ...
          {!isFinalYear ? <div className="promotion-final-actions"><button className="cms-btn cms-btn-primary" onClick={openPreview} disabled={!selectedStudents.length || previewLoading || submitting}>{previewLoading ? "Preparing Preview..." : "Preview Promotion"}</button></div> : null}
        </> : null}
