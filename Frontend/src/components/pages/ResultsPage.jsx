...
export default function ResultProcessingPage() {
  ...
  const exportExcel = async (rows, filename) => {
    ...
    const examId = selectedPublishedGroup?.examId || applied.exam || filters.exam;
    const boardId = selectedPublishedGroup?.boardId || applied.board || filters.board;
    const academicYearId = selectedPublishedGroup?.academicYearId || applied.year || filters.year;
    const academicLevelId = selectedPublishedGroup?.academicLevelId || applied.level || filters.level;
    const groupId = selectedPublishedGroup?.groupId || applied.group || filters.group;
    ...
    try {
      ...
      const exportData = sorted.map((item, idx) => ({
        ...
        Group: item.groupName || selectedPublishedGroup?.groupName || "—",
