...
import { courses as groupMasterData } from "@/data/mockData.js";
import "./SubjectManagementPage.css";

const groups = Object.keys(groupSubjectMap).map((code) => ({
  code,
  name: groupMasterData.find((group) => String(group.code).toLowerCase() === code.toLowerCase())?.name || code,
}));
...
export default function SubjectManagementPage() {
  ...
  const assigned = useMemo(() => mapping[context.group] ?? [], [mapping, context.group]);
...
function GroupCombobox({ value, onChange }) {
  ...
  const matches = groups.filter((group) => (group.name + " " + group.code).toLowerCase().includes(query.toLowerCase()));
  ...
  const choose = (group) => { onChange(group.code); setOpen(false); setActive(0); };
  const keyDown = (event) => {
